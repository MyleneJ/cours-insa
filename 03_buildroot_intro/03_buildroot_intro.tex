...
\begin{frame}{Utilisation et création d'un {\em defconfig}}
  ...
  \item Buildroot permet de charger un {\em defconfig} issu du dossier
    ...
    \begin{itemize}
    \item Ca va surcharger le \code{.config}, si il y en a un
    \end{itemize}
...
\begin{frame}{Source tree (2/3)}
  ...
  \item \code{boot/} : paquets de bootloarder (\code{barebox}, \code{uboot}, etc)
  \end{itemize}
\end{frame}
...
\begin{frame}{Déploiement des images}
  \begin{itemize}
  \item Par défaut, Buildroot stockent les images dans \code{output/images}
...
\begin{frame}{Déploiement des images : genimage}
  \begin{itemize}
  \item \code{genimage} : peut créer des image complète pour des périphériques
